...
# This means that output location will be _inferred_ as '$(GO_PROTOS_DIR)/opts'
	...
	--go_out=plugins=grpc:$(GO_PROTOS_DIR) \
	--go_opt=paths=source_relative \
	protos/opts/ps_opts_connection.proto

	docker run --rm -w $(PWD) -v $(PWD):$(PWD) -w${PWD} jaegertracing/protobuf:0.2.0 \
	--proto_path=./protos \
	--go_out=plugins=grpc:$(GO_PROTOS_DIR) \
	--go_opt=paths=source_relative \
	protos/opts/ps_opts_read.proto

	docker run --rm -w $(PWD) -v $(PWD):$(PWD) -w${PWD} jaegertracing/protobuf:0.2.0 \
	--proto_path=./protos \
	--go_out=plugins=grpc:$(GO_PROTOS_DIR) \
	--go_opt=paths=source_relative \
	protos/opts/ps_opts_write.proto
	...
	--go_out=plugins=grpc:$(GO_PROTOS_DIR) \
	--go_opt=paths=source_relative \
	protos/opts/ps_opts_streamdal.proto
	...
	--go_out=plugins=grpc:$(GO_PROTOS_DIR) \
	--go_opt=paths=source_relative \
	protos/opts/ps_opts_tunnel.proto

	docker run --rm -w $(PWD) -v $(PWD):$(PWD) -w${PWD} jaegertracing/protobuf:0.2.0 \
	--proto_path=./protos \
	--go_out=plugins=grpc:$(GO_PROTOS_DIR) \
	--go_opt=paths=source_relative \
	protos/opts/ps_opts_relay.proto
