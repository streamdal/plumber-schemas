...
const (
	grepFile = "build/go/protos/opts/ps_opts_relay.pb.go"
	outputFile = "./build/go/protos/opts/batch_merge_relay_opts.pb.go"
)
...
func main() {
	...
	grepArgs := [][]string {
		{`\*RelayGroup.\+protobuf:`, path + "/"  + grepFile},
	}

	// key = backend name
	mappings := make(map[string]MergeMapping, 0)
	...
	for _, cmd := range grepArgs {
		...
		for _, line := range lines {
			...
			r, err := regexp.Compile(`\t?([a-zA-Z0-9]+)\s+\*RelayGroup(.+)Options\s+.+`)
			if err != nil {
	...
	fileContents, err := createRelayCode(mappings)
	if err != nil {
		log.Fatalf("unable to create file contents: %s", err)
	}

	if err := writeMergeCode(outputFile, fileContents); err != nil {
		log.Fatalf("unable to write file '%s': %s", outputFile, err)
	}

	fmt.Printf("Successfully generated file '%s' with '%d' relays\n", outputFile, len(mappings))
}

func createRelayCode(mappings map[string]MergeMapping) ([]byte, error) {
